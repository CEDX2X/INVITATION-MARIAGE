      ...
      <div className="max-w-xl w-full text-center">
        ...
        {timeLeft.isExpired ? (
          ...
          <div className="relative max-w-4xl mx-auto w-full">
            {/* Dark-to-light decorative background behind the countdown (like the screenshot) */}
            <div
              className="absolute inset-0 -z-10 rounded-3xl"
              style={{
                background: 'linear-gradient(180deg, rgba(10,6,10,0.95) 0%, rgba(70,10,30,0.6) 30%, rgba(250,250,250,0.98) 100%)',
              }}
            />

            <div className="grid grid-cols-4 gap-2 sm:gap-4 max-w-md mx-auto p-2 sm:p-6 rounded-3xl">
              {timeBlocks.map((block, index) => (
                ...
                >
                  <span className="font-serif font-light text-2xl sm:text-3xl md:text-4xl text-rose-800 tracking-tight">
                    {block.value.toString().padStart(2, "0")}
                  </span>
                  <span className="text-[10px] sm:text-xs uppercase tracking-widest text-rose-300 font-sans font-medium mt-2">
                    {block.label}
                  </span>
                </motion.div>
              ))}
